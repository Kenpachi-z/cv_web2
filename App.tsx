...
const App: React.FC = () => {
  ...
  const handleExportHtml = () => {
    ...
    // Clone le CV pour manipulation
    const cvClone = cvElement.cloneNode(true) as HTMLElement;
    
    // Forcer l'injection de la photo si elle existe
    ...
    // Nettoyage des éléments non souhaités dans l'export
...
<head>
    ...
    <style>
        ...
        /* Assurer le style du lien LinkedIn exporté */
        a[href*="linkedin"] {
            color: ${THEME_COLOR} !important;
            text-decoration: none !important;
        }
        a[href*="linkedin"] span {
            border-bottom: 1px solid rgba(131, 16, 16, 0.3) !important;
        }
    ...
    <div className="min-h-screen flex flex-col items-center sm:py-6 px-0 sm:px-4 font-sans text-slate-900 selection:bg-red-100">
      ...
      <nav className="max-w-[210mm] w-full mb-6 flex flex-col sm:flex-row gap-4 justify-between items-center no-print px-4 sm:px-0">
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setIsCLModalOpen(true)} className="flex items-center gap-2 bg-white border border-slate-300 px-4 py-2 rounded-lg font-bold text-xs hover:bg-slate-50 shadow-sm transition-all">
            <i className="fa-solid fa-wand-magic-sparkles text-amber-500"></i> Adaptation IA
          </button>
        </div>
        ...
        <div className="flex gap-2">
          <button 
            ...
            title="Télécharger le fichier HTML pour consultation hors-ligne"
          >
            ...
            <main className="main-col flex-1 p-6 sm:p-10 overflow-hidden bg-white">
              ...
              <header className="flex gap-8 items-start mb-10">
                ...
                <div className="flex-1 pt-4">
                  <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-2 leading-none">
                    {RESUME_DATA.name}
                  </h1>
                  <h2 className="text-md font-bold mb-6 leading-tight max-w-[420px]" style={{ color: THEME_COLOR }}>
                    {RESUME_DATA.title}
                  </h2>
              ...
              <section className="mb-10">
                <div className="bg-slate-50 border-l-[4px] p-6 rounded-r-2xl relative overflow-hidden" style={{ borderLeftColor: THEME_COLOR }}>
                  {/* Guillemets décoratifs ajoutés ici */}
                  ...
                  <p className="text-slate-800 leading-relaxed text-[13px] font-semibold italic relative z-10">
                    "{RESUME_DATA.profile}"
                  </p>
                </div>
              </section>
                  ...
                  {RESUME_DATA.experiences.map((exp, idx) => (
                    <article key={idx} className="relative pl-8 break-inside-avoid">
                      ...
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline mb-1 gap-1">
                        ...
                        <span className="text-[10px] font-black text-white px-2.5 py-1 rounded-full uppercase tracking-widest shadow-sm" style={{ background: GRADIENT_PRIMARY }}>
                          {exp.period}
                        </span>
                      </div>
                      
                      <div className="font-bold mb-3 text-[12px] tracking-widest uppercase" style={{ color: THEME_COLOR }}>
                        {exp.company}
                      </div>
      ...
      <AIChatAssistant />
      {isCLModalOpen && <CoverLetterModal onClose={() => setIsCLModalOpen(false)} />}
